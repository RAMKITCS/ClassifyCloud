...
def update(id,que,doc_type,pred_score):
    return Connection().update_one({'_id':id},{"$set":{'queue':que,'doc_type':doc_type,"pred_score":pred_score,"completed_date":datetime.now().strftime(("%d/%m/%Y %H:%M:%S"))}})
def delete(query):
